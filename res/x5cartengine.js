...
_jq.extend(x5engine.imCart, {
	...
	_getCart: function () {
		try {
			if (_jq.imCookie(x5engine.imCart.costants.COOKIE_NAME, null, {
				path: '/'
			}) != null) return _jq.parseJSON(_jq.imCookie(x5engine.imCart.costants.COOKIE_NAME, null, {
				path: '/'
			}));
			else return [];
		} catch (e) {
			_jq.imCookie(x5engine.imCart.costants.COOKIE_NAME, {}, {
				path: '/'
			});
			return [];
		}
	...
	_getShippingPrice: function (shipping, subtotal, total_quantity, total_weight) {
		if (shipping.price != null) {
			...
			if (shipping.vat != null) price += price * shipping.vat;
			return price;
		}
